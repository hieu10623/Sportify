...
app.controller("shopping-cart-ctrl", function ($scope, $http, $cookies) {
    $scope.cart = {
        ...
        get cartCount() {
            return this.items.map(item => item.buying).reduce((total, buying) => total += buying, 0);
        },
        //tính tổng tiền của giỏ hàng
        get totalPrice() {
            return this.items.map(item => item.buying * (item.price - item.discountprice)).reduce((total, buying) => total += buying, 0);
        },
        ...
        get orderDetails() {
            return $scope.cart.items.map(item => {
                return {
                    ...
                    buying: item.buying,
                    //producTotalPrice: item.price * item.buying 
                }
            });
        },
        purchase() {
